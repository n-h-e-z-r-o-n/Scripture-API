...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (wantsRandom) {
      const allEntries = categories.flatMap((category) =>
      ...
      const motivations = [...allEntries]
        .sort(() => Math.random() - 0.5)
        .slice(0, limit);

      return jsonResponse({
        totalCategories: categories.length,
        totalMotivations: allEntries.length,
    ...
    return jsonResponse({
      ...
      categories: categories.map((category) => ({
        ...
        href: `/api/motivations/${category.slug}`,
      })),
    });
  } catch (error) {
    console.error('Failed to read motivation categories', error);
    return errorResponse(500, 'Failed to load motivations');
  }
}
